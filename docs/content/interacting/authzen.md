...
If you want to use the Get Configuration discovery endpoint, you must also configure the canonical base URL that OpenFGA should publish in discovery metadata:

```shell
openfga run --experimentals=authzen --authzen-base-url=https://api.example.com
```

Or via environment variable:

```shell
OPENFGA_EXPERIMENTALS=authzen OPENFGA_AUTHZEN_BASE_URL=https://api.example.com openfga run
```
...
The returned absolute URLs are built from the configured `authzen.baseURL` value, not from the incoming request's `Host`, `X-Forwarded-Host`, or similar headers. This ensures discovery metadata stays bound to the canonical origin you configured.
...
If `authzen.baseURL` is not configured, the discovery endpoint returns an error instead of inferring a base URL from request headers.
