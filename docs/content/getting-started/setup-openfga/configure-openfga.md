...
* requirements
  * [install OpenFGA](../overview.md)
  * `openfga` binary | your PATH

* goal
  * configure the standalone OpenFGA server
    * if you are using OpenFGA as a library -> see [GoDoc](https://pkg.go.dev/github.com/openfga/openfga)

## Configuring data storage

* supported storage engine options
  * `memory`
    * == memory storage engine
    * default one
    * data is ephemeral
      * == | restart the OpenFGA server, data is lost
    * ❌NOT use cases ❌
      * production
  * `postgres`
...
* | run OpenFGA by FIRST time, OR install a NEW version,
  * run `openfga migrate`
    * -> 
      * create the required database tables
      * perform the database migration / NEW version
...
OpenFGA supports configuring separate read and write datastores for PostgreSQL to improve performance and scalability
* This feature allows you to distribute read operations across read replicas while directing write operations to the primary database.
