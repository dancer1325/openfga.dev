...
* requirements
  * [OpenFGA v1.7.0](https://github.com/openfga/openfga/releases/tag/v1.7.0)

* == built-in access control feature /
  * allows you to
    * control access -- to -- your OpenFGA server
  * relies -- on a -- control store / 
    * authorize requests -- , based on its OWN model & tuples, to -- the OpenFGA server
  * ❌limitations == NO way to❌
    * initialize the
      * access control store
      * access control model
    * bootstrap the client IDs / are supposed to be admins
  * ⚠️experimental⚠️
...
* OIDC Provider
  * You need to have an OIDC provider [set up and configured](./configure-openfga.mdx#oidc) in your OpenFGA server set up to use access control.
...
- The FGA CLI: While the CLI is not strictly required, you need to follow the steps below
    * You can install it by following the instructions [here](../cli.mdx)
    * If you do not want to use the CLI, you can call the API with the equivalent SDK or REST calls.

### 1. Ensure the server is running (with access control disabled)

This is important
* If you enable access control before setting up the store and model and grant your initial client ID access, you will lock yourself out of the server, and you will have to turn it back off.

### 2. Create the access control store and model
...
#### iii. Restart the OpenFGA server

### 4. Grant access to a store
